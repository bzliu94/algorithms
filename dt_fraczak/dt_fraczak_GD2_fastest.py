...
def GD(V, A, g_start_node):
  ...
  t_edges = t.getEdges()
  next_t = UnionTree()
  next_vertices = [UnionTreeVertex(name_to_g_vertex_dict[x.getName()]) for x in t_vertices]
  name_to_next_tree_vertex_dict = {}
  for next_vertex in next_vertices:
    name_to_next_tree_vertex_dict[next_vertex.getName().getName()] = next_vertex
  for t_edge in t_edges:
    old_origin = t_edge.getOrigin()
    old_dest = t_edge.getDestination()
    next_origin = name_to_next_tree_vertex_dict[old_origin.getName()]
    next_dest = name_to_next_tree_vertex_dict[old_dest.getName()]
    next_dest.setParent(next_origin)
    next_origin.addChild(next_dest)
  next_t.setRoot(name_to_next_tree_vertex_dict[g_start_node.getName()])
  ...
  for u_num in reversed(xrange(1, num_nodes + 1)):
    ...
    for edge in arcs[u]:
      ...
      out_bag[uf_gt.find(x)].addLast(y_dll_node)
      in_bag[uf_gt.find(y)].addLast(x_dll_node)
      added[uf_gt.find(y)] += 1
