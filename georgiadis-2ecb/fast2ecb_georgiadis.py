...
# fixed problem with step 3.2 in modified Simple2ECB 
# being time in O(m * n) instead of O(m)

# 2019-05-25

# time is O((m + n) * alpha)

# we implement fast2ecb from georgiadis et al. 2014

# for layer three, there may be more than one strong bridge left 
# (ignoring overall ordinary status), but we care about 
# at most one of them only
...
def doFast2ECB(V, E):
  result = doRec2ECBHelper(V, E, DO_FORWARD)
  return result.toElementList()
def doRec2ECBHelper(V, E, phase = DO_FORWARD):
    ...
    for subtree_root, curr_subtree_G, subtree_node_group in augmented_subtree_graphs:
      ...
      for edge in shiny_edges:
        if edge.toString() not in seen_edge_str_set:
          ...
          seen_edge_str_set |= set([edge.toString()])
      curr_subtree_G.addEdges(next_shiny_edges)
  subtree_root_subtree_graph_tuples = [(x[0], x[1]) for x in augmented_subtree_graphs]
  next_phase = None
  results = None
  if phase == DO_FORWARD:
    next_phase = DO_REVERSE
  elif phase == DO_REVERSE:
    next_phase = DO_SIMPLE_LIGHT
  if next_phase == DO_REVERSE:
    results = [doRec2ECBHelper(x[1].getVertices(), x[1].getEdges(), next_phase) for x in subtree_root_subtree_graph_tuples]
  elif next_phase == DO_SIMPLE_LIGHT:
    # we must specify the strong bridge we are looking for third layer
    results = []
    for curr_subtree_root, curr_subtree_graph in subtree_root_subtree_graph_tuples:
      anticipated_sb_origin_name = None
      anticipated_sb_dest_name = None
      if source_subtree.getRoot().getName() != curr_subtree_root.getName():
        # an in-edge may exist that we care about
        anticipated_sb_dest_name = curr_subtree_root.getName()
        anticipated_sb_origin_name = source_subtree.getVertexByName(anticipated_sb_dest_name).getParent().getName()
      curr_result = doSimple2ECBModified(curr_subtree_graph.getVertices(), curr_subtree_graph.getEdges(), anticipated_sb_origin_name, anticipated_sb_dest_name)
      results.append(curr_result)
  if len(results) == 0:
    raise Exception()
  else:
...
# we only attempt using a strong bridge with origin and destination 
# that each have certain given names, if it exists
def doSimple2ECBModified(V, E, anticipated_sb_origin_name, anticipated_sb_dest_name):
  # if there is low number of ordinary nodes left, do not attempt core of simple2ecb
  overall_ordinary_nodes = [x for x in V if x.isOverallOrdinary() == True]
  if len(overall_ordinary_nodes) == 0:
    return DoublyLinkedList()
  elif len(overall_ordinary_nodes) == 1:
    result = DoublyLinkedList()
    dll_node = DoublyLinkedListNode(overall_ordinary_nodes, None, None)
    result.addLast(dll_node)
    return result
  # step one
  s = V[0]
  # we're allowed to find all strong bridges 
  # even though we may ignore all or all except one 
  # because we are allowed to take time 
  # at least linear in number of nodes and edges
  strong_bridges = doStrongBridges(V, E, s)
  next_strong_bridges = []
  for strong_bridge in strong_bridges:
    origin = strong_bridge.getOrigin()
    dest = strong_bridge.getDestination()
    origin_name = origin.getName()
    dest_name = dest.getName()
    if origin_name == anticipated_sb_origin_name and dest_name == anticipated_sb_dest_name:
      next_strong_bridges.append(strong_bridge)
  # step two
  curr_blocks = [V[ : ]]
  # step three
  n = len(V)
  ...
  for curr_sb in next_strong_bridges:
    # step 3.1
  ...
  # extra relating to doubly-linked lists
  next_result = DoublyLinkedList()
  for curr_block in curr_blocks:
    next_curr_block = [x for x in curr_block if x.isOverallOrdinary() == True]
    if len(next_curr_block) != 0:
      dll_node = DoublyLinkedListNode(next_curr_block, None, None)
      next_result.addLast(dll_node)
  return next_result
...
if __name__ == '__main__':
  ...
  blocks = doFast2ECB(vertices, edges)
