...
from dsuf_gt import UnionTreeUnionFind, UnionTree, UnionTreeVertex
...
def doIntervalsHelper(g, t, h, I, uf_gt, v, i, augmenting_g_edge_container):
  if v <= i:
...
def doIntervals(g, t, adj, g_start_node):
  ...
  V = g.getVertices()
  name_to_g_vertex_dict = {}
  for g_vertex in V:
    name_to_g_vertex_dict[g_vertex.getName()] = g_vertex
  t_vertices = t.getVertices()
  t_edges = t.getEdges()
  next_t = UnionTree()
  next_vertices = [UnionTreeVertex(x.getPreorderNumber()) for x in t_vertices]
  num_to_next_tree_vertex_dict = {}
  for next_vertex in next_vertices:
    num_to_next_tree_vertex_dict[next_vertex.getName()] = next_vertex
  for t_edge in t_edges:
    old_origin = t_edge.getOrigin()
    old_dest = t_edge.getDestination()
    next_origin = num_to_next_tree_vertex_dict[old_origin.getPreorderNumber()]
    next_dest = num_to_next_tree_vertex_dict[old_dest.getPreorderNumber()]
    next_dest.setParent(next_origin)
    next_origin.addChild(next_dest)
  next_t.setRoot(num_to_next_tree_vertex_dict[g_start_node.getPreorderNumber()])
  uf_gt = UnionTreeUnionFind(next_t)
  ...
  for i in xrange(num_nodes, 2 - 1, -1):
    ...
    for curr_g_edge in curr_g_edges_for_reintroduction:
      ...
      augmenting_g_edge = AugmentingEdge(g_v, g.getVertexByName(t.getVertexByOIPN(uf_gt.find(t_w_oipn)).getName()))
    ...
    for curr_g_cycle_edge in curr_g_cycle_edges:
      ...
      curr_value = h[uf_gt.find(t_oipn)]
      if curr_value == 1 and curr_g_cycle_edge.isActive() == True:
        doIntervalsHelper(g, t, h, I, uf_gt, uf_gt.find(t_oipn), i, augmenting_g_edge_container)
  # phase three
    ...
    if True:
      for u in I[i]:
        ...
        for curr_g_edge in wake_list[u]:
          ...
          if uf_gt.find(t_v_oipn) != uf_gt.find(t_w_oipn):
            v_star[curr_g_edge] = uf_gt.find(t_v_oipn)
            w_star[curr_g_edge] = uf_gt.find(t_w_oipn)
            l[curr_g_edge] = i
