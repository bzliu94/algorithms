...
class RTree:
  ...
  def construct(points):
    ...
    entry = RTree.constructHelper(tree, root_entry, points, m, M, r)
    node = entry.getChild()
    tree.setRootEntry(entry)
    return tree
    # d-way split
    # d <= M
    # choose dimension based on variance for nodes
  # constructs a subtree for given points
  # returns a root entry
  @staticmethod
  def constructHelper(tree, entry, points, m, M, r):
    node = entry.getChild()
    # parent_node = parent_entry.getChild()
    # print len(points), M
    if len(points) <= M:
      # make nodes, entries, raw mbr's for each point object
      entry_list = []
      # node = RTreeNode(parent_node, [], True)
      # entry = RTreeEntry(None, node)
      # node = curr_node
      # entry = curr_entry
      for point in points:
        curr_location = point.toLocationTuple()
        curr_node = RTreeNode(node, [], True)
        curr_mbr = RawMBR(curr_location, curr_location, point)
        curr_entry = RTreeEntry(curr_mbr, curr_node)
        entry_list.append(curr_entry)
      node.addEntries(entry_list)
      mbr_list = [x.getMBR() for x in entry_list]
      mbr = CompositeMBR.makeMBR(mbr_list)
      entry.setMBR(mbr)
      # parent_mbr = parent_entry.getMBR()
      return entry
    else:
      point_x_values = [x.getX() for x in points]
      x_variance = shifted_data_variance(point_x_values)
      point_y_values = [x.getY() for x in points]
      y_variance = shifted_data_variance(point_y_values)
      chosen_axis = 0 if x_variance >= y_variance else 1
      sorted_points = points[ : ]
      if chosen_axis == 0:
        sorted_points.sort(key = lambda x: x.getX())
      elif chosen_axis == 1:
        sorted_points.sort(key = lambda x: x.getY())
      size = int(math.ceil(r / (1.0 * M)))
      eff_M = int(math.ceil(r / (1.0 * size)))
      # print eff_M
      entries = []
      for i in range(eff_M):
        next_node = RTreeNode(node, [], False)
        next_entry = RTreeEntry(None, next_node)
        point_group = sorted_points[i * size : (i + 1) * size]
        curr_entry = RTree.constructHelper(tree, next_entry, point_group, m, M, len(point_group))
        entries.append(curr_entry)
      node.addEntries(entries)
      mbr_list = [x.getMBR() for x in entries]
      next_mbr = CompositeMBR.makeMBR(mbr_list)
      entry.setMBR(next_mbr)
      return entry  
      ...
      while entry_pq.getSize() != 0:
        ...
        if node.isLeafNode():
          ...
          if dist <= TopicKNearest.getFarthestCloseDistance() + 0.001 or TopicKNearest.isFull() == False:
            ...
            # id_value = point_to_id_dict[location][0]
            # TopicKNearest.addAndRemoveIfNecessary(close_item)
